...
summary(m_species)
check_overdispersion(m_species) #no overdipersion
...
hist(resid(m_species)) #distribution of residuals has tail to negative values
...
ggplot(comb_all2, aes(x = Stems, y = predictions, color = as.factor(Group))) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "nbinom1", theta = theta_value)) +
